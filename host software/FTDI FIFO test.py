...
def connect():
 ...
 if deviceIndex >= 0 :
     ...
     time.sleep(0.1)
     #dev.setBitMode(0x00, 0x40) #think this is synchronous FIFO
...
def send():
    ...
    if len(tx_data)>0 :
        TB2.insert(END, "\r\nSending %d bytes:\r\n"%len(tx_data))
        TB2.insert(END,'-------------------------------------------------\r\n')
        TB2.insert(END,bytearray(tx_data))
        TB2.insert(END,'\r\n')
        TB2.insert(END,'-------------------------------------------------\r\n')
        TB2.see("end")
...
def read():
    ...
    if len(rx_buffer)>0 :
        ...
        TB2.insert(END,'-------------------------------------------------\r\n')
        # TB2.insert(END,)
        for indx in range(min(1000, len(rx_buffer))):
            TB2.insert(END,rx_buffer[indx])
            TB2.insert(END,' ')
        TB2.insert(END,rx_buffer[0:min(1000,len(rx_buffer))])
        TB2.insert(END,'\r\n')
        TB2.insert(END,'-------------------------------------------------\r\n')
        TB2.see("end")
...
def timerHandler(): # must be after ftdi functions, before buttons callback
    read()
    if RxTimerState > 0 :
        timer1 = threading.Timer(10, timerHandler).start()
